...
function main() {
    const nim = 2211104072; // Ganti dengan NIM Anda
    const game = new PosisiKarakterGame(nim);

    // Simulasi perubahan state sesuai dengan urutan pada gambar
    game.changeState('TombolW'); // tombol arah atas ditekan (jika NIM % 3 == 0)
    game.changeState('TombolS'); // tombol arah bawah ditekan (jika NIM % 3 == 0)
    ...
    game.changeState('TombolS');
    game.changeState('TombolW');
    game.changeState('TombolX'); // Tidak ada aksi yang sesuai
}
