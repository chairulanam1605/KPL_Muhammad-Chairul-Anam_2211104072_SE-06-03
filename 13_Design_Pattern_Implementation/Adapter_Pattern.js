...
class IndonesianToEnglishTranslator extends EnglishSpeaker {
    ...
    speak(message) {
        ...
        const spokenInIndonesian =
            this.indonesianSpeaker.berbicaraDalamIndonesia(indonesianMessage);
...
englishMessages.forEach((message) => {
    ...
    console.log(
        `Penerjemah menyampaikan ke delegasi Indonesia: "${translatedToIndonesian}"`
    );
    console.log(
        `Delegasi Indonesia merespon dalam bahasa Indonesia: "${translatedToIndonesian}"`
    );
    // Menerjemahkan kembali ke bahasa Inggris
    const translatedBackToEnglish = translator.translateToEnglish(
        translatedToIndonesian
    );
    console.log(
        `Penerjemah menerjemahkan kembali ke bahasa Inggris: "${translatedBackToEnglish}"`
    );
});
